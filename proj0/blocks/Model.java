...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        if (piece == null) {
            return false;
        } else if (piece.get(0, 0)) {
            for (int i = 0; i < piece.height(); i++) {
                for (int x = 0; x < piece.width(); x++) {
                    if (row + i >= _height || col + x >= _width) {
                        return false;
                    }
                    if (piece.get(i, x)) {
                        if (get(row + i, col + x)) {
    ...
    int[][] rowColumnCounts() {
        int[][] result = new int[][] { new int[_width], new int[_height] };
        for (int i = 0; i < _height; i++) {
            int count = 0;
            for (int x = 0; x < _width; x++) {
                if (_cells[i][x]) {
    ...
    void clearFilledLines() {
        ...
        for (int x = 0; x < counts[1].length; x++) {
            if (counts[1][x] == _height) {
                ncols += 1;
                for (int i = 0; i < _height; i++) {
                    _cells[i][x] = false;
                    _score++;
        ...
        if (nrows >= 1 && ncols >= 1) {
            _score -= java.lang.Math.max(nrows, ncols);
        }
        if (nrows > 0 || ncols > 0) {
            _streakLength++;
        } else {
        ...
        _score += _streakLength * scoreClearedLines(nrows, ncols);
